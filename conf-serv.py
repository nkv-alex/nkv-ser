...
def detect_interfaces():
    ...
    for line in out.splitlines():
        ...
        tipo = input(f"Is this interface internal (i) or external (e)? [{suggested}]: ")
...
def autoconfig_dns():
    ...
    iface_ip = interfaces[iface]['ip'].split('/')[0]

    print("[INFO] Writing dnsmasq configuration...")
    conf_data = f"""# Generated by conf-serv.py
domain={domain}
interface={iface}
...
"""
    ...
    for dns in dns_range.split(','):
        conf_data += f"server={dns.strip()}\n"
    ...
    if "active" in status.stdout:
        ...
        print(f"[INFO] DNS listening on: {iface_ip}")
    else:
                    ...
                    match Z:
                        ...
                        case 5:
                            Z2= int(input("\nSelect an option:\n"
                                        ...
                                        "7. Restore smb.conf\n"
                                        "8. Restore exports\n"
                                        "9. Restore dnsmasq.conf\n"
                                        "Option\n> "))
                            match Z2:
                                case 1:
                                    restore_file("/etc/netplan/01-nat.yaml")
                                case 2:
                                ...
                                case 5:
                                    restore_file("/etc/apache2/sites-available/default-ssl.conf")
                                case 6:
